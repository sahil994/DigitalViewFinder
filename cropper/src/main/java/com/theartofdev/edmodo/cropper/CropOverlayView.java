...
public class CropOverlayView extends View {
     ...
     */
    private final float[] mBoundsPoints = new float[11];
...
*/
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCropWindowHandler.showGuidelines()) {
            ...
            if (mGuidelines == CropImageView.Guidelines.ON) {
                rows = CropImage.mRows;
                column = CropImage.mColumn;
    ...
    private void drawGuidelines(Canvas canvas) {
        if (mGuidelinePaint != null) {
            ...
            float oneThirdCropWidth = rect.width() / rows;
            float oneThirdCropHeight = rect.height() / column;
            ...
            if (mCropShape == CropImageView.CropShape.OVAL) {
                ...
                for (int i = 0; i < column; i++) {
                    ...
                    //       canvas.drawLine(x3, rect.top, x3, rect.bottom, mGuidelinePaint);
                    //     canvas.drawLine(x2, rect.top, x2, rect.bottom, mGuidelinePaint);


                    x1 = x1 + oneThirdCropWidth;
